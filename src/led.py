...
class Point2D:
    def __init__(self, u: float, v: float, contours=(), brightness: float = 0.0):
...
def merge(leds: list[LED3D]) -> LED3D:
    ...
    # Collect brightness values from detections
    brightnesses = []
    for led in leds:
        if len(led.detections) > 0:
            # Use the average brightness if multiple detections for this LED
            avg_brightness = np.mean([det.point.brightness for det in led.detections])
            brightnesses.append(avg_brightness)
        else:
            # If no detections, use equal weight
            brightnesses.append(1.0)

    # If all brightnesses are zero, use equal weighting
    if sum(brightnesses) == 0:
        brightnesses = [1.0] * len(brightnesses)

    # Use brightness-weighted average for position (particularly important for Z-axis)
    new_led.point.position = np.average([led.point.position for led in leds], axis=0, weights=brightnesses)
    ...
    # Combine detections from all merged LEDs
    new_led.detections = [det for led in leds for det in led.detections]
