...
def find_led_in_image(image: np.ndarray, threshold: int = 128) -> Optional[Point2D]:
    ...
    # Calculate brightness of the brightest contour
    brightness_tuple = contour_brightness(image, brightest_contour)
    # cv2.sumElems returns a tuple, take the first element (grayscale sum)
    brightness = float(brightness_tuple[0]) if isinstance(brightness_tuple, tuple) else float(brightness_tuple)
    ...
    return Point2D(center_u, center_v, contours, brightness=brightness)  # todo, normalise contours
