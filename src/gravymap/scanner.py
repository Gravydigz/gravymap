...
from gravymap.sfm_process import SFM

from tqdm import tqdm
from pathlib import Path
from gravymap.detector_process import DetectorProcess
from gravymap.queues import Queue2D, DetectionControlEnum
from multiprocessing import get_logger, set_start_method
from gravymap.file_tools import get_all_2d_led_maps
from gravymap.utils import get_user_confirmation
from gravymap.visualize_process import VisualiseProcess
from gravymap.led import last_view
from gravymap.file_writer_process import FileWriterProcess
...
class Scanner:
    ...
    def __init__(
        ...
        self.rotation_angle = 360.0 / num_views if num_views else None
    ...
    def mainloop(self):
        ...
        while True:
            ...
            # Check if we've reached the desired number of views
            if self.num_views and self.current_view >= self.num_views:
                print(f"\nCompleted all {self.num_views} views!")
            ...
            if success:
                ...
                # Display rotation guidance if num_views is set
                if self.num_views and self.current_view < self.num_views:
                    print(f"\nScan {self.current_view}/{self.num_views} complete.")
                    print(f"Rotate camera {self.rotation_angle:.1f}° clockwise and press Enter to continue.")
                elif self.num_views:
                    print(f"\nScan {self.current_view}/{self.num_views} complete.")
